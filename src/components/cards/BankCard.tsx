...
export function BankCard({ styleType, cardInfo }: BankCardProps) {
  ...
  return (
    <div className={cardStyles[styleType]}>
      ...
      <p className="text-4xl pt-1 font-bold">${cardInfo.balance.toFixed(2)}</p>
      <p className="absolute bottom-5 left-6 text-xs">{cardInfo.name}</p>
      {styleType === "visa" && (
